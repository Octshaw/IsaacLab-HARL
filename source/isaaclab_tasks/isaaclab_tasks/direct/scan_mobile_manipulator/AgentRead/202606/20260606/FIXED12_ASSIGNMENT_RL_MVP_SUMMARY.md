...
```text
source/isaaclab_tasks/isaaclab_tasks/direct/scan_mobile_manipulator/AgentRead/202606/20260604/ASSIGNMENT_BASED_RL_INTERFACE_DESIGN.md
source/isaaclab_tasks/isaaclab_tasks/direct/scan_mobile_manipulator/AgentRead/202606/20260605/ASSIGNMENT_BASED_RL_PHASE2_HARL_DISCRETE_SHAPE_REPORT.md
source/isaaclab_tasks/isaaclab_tasks/direct/scan_mobile_manipulator/AgentRead/202606/20260606/PHASE5_FIXED12_EVAL_ANALYSIS_REPORT.md
```
...
## 10. Regression Baseline Role

The fixed-12 MVP should remain available as a regression scenario.

Future changes to component proxy loading, viewpoint file loading, and feasibility generation should not break the fixed-12 assignment-RL path.

Before major refactors, rerun a bounded fixed-12 smoke/eval to confirm:

```text
assignment-RL still reaches 100% coverage
baseline solvers still run
available_actions shape remains valid
feasible_mask semantics remain valid
```
...
not arbitrary viewpoint-set generalization
not a real robot articulation benchmark
not a replacement for future feasibility generation
```

The existing conclusions remain:

```text
fixed-12 MVP does not prove arbitrary viewpoint-set generalization
robot_2 -> viewpoint_5 is a scenario-level override
