...
$taskGeneratedOutputPaths=@('execution_authority_single_absence_final.json','execution_result_single_absence_final.json','deletion_log_single_absence_final.jsonl')|ForEach-Object {Join-Path $taskExecutionRoot $_}
...
foreach($name in @('execution_authority_single_absence_final.json','deletion_log_single_absence_final.jsonl','execution_result_single_absence_final.json')){
...
function Repository-Snapshot{
 ...
 if($rel -eq '.git' -or $item.FullName -in $taskGeneratedOutputPaths){continue}
...
$taskAuthority=[ordered]@{
 ...
 authorization_source='Latest explicit user supplement: sole phase_b_git_closeout_artifacts.zip ALREADY_ABSENT; original attachment authority remains except that precise absence.'
 ...
 originally_approved_files=50639;originally_approved_logical_bytes=12122958520
 already_absent_files=1;already_absent_manifest_bytes=36178
 already_absent_relative_path='source/isaaclab_tasks/isaaclab_tasks/direct/scan_mobile_manipulator/AgentRead/202609/20260925/phase_b_git_closeout_artifacts.zip'
 effective_delete_files=50638;effective_delete_logical_bytes=12122922342;added_targets=0
 authorized_files=50638;authorized_logical_bytes=12122922342
 ...
 adapted_validator_sha256='e25f89256ee96578687bf91e73b81c9e163e35355a7da14f13f6746d444a3095'
...
$taskLogStream=[IO.FileStream]::new((Join-Path $taskExecutionRoot 'deletion_log_single_absence_final.jsonl'),[IO.FileMode]::CreateNew,[IO.FileAccess]::Write,[IO.FileShare]::Read,4096,[IO.FileOptions]::WriteThrough)
...
$taskVerifier = Join-Path $taskExecutionRoot 'verify_delete_allowlists_single_absence.ps1'
$taskApprovedVerifierHash = 'e25f89256ee96578687bf91e73b81c9e163e35355a7da14f13f6746d444a3095'
...
if($taskVerified.Summary.already_absent_files -ne 1 -or $taskVerified.Summary.added_targets -ne 0 -or
 $taskVerified.Summary.already_absent_relative_path -cne 'source/isaaclab_tasks/isaaclab_tasks/direct/scan_mobile_manipulator/AgentRead/202609/20260925/phase_b_git_closeout_artifacts.zip'){
 throw 'Single-file exception proof mismatch.'
}
...
foreach($name in @('delete_strong_allowlist.json','delete_recommended_allowlist.json','keep_required.json','keep_compact_evidence.json','keep_local_useful.json','unknown_review_required.json','archive_optional_allowlist.json')){
 ...
 $taskKeepBaseline[$manifest.classification]=[ordered]@{files=$members.Count;bytes=$(if($members.Count -eq 0){0L}else{[long](($members|Measure-Object size_bytes -Sum).Sum)});audit_files=$manifest.totals.files;audit_bytes=$manifest.totals.bytes;members=$members.ToArray()}
}
...
Save-NewJson 'execution_authority_single_absence_final.json' $taskAuthority
Write-Output 'PRECHECK: running one-exception validator bound Recheck immediately before approved deletion.'
...
if ($taskVerified.Files.Count -ne 50638 -or
    $taskVerified.Summary.verified_exact_size_bytes -ne 12122922342) {
...
$taskFinalQuiet=Assert-Quiescent
$taskAuthority.bound_recheck_summary=$taskVerified.Summary
Record-Event ([ordered]@{event='bound_recheck_pass';summary=$taskVerified.Summary;utc=[DateTime]::UtcNow.ToString('o')})
$taskStage='DELETE'
...
foreach ($taskFile in $taskVerified.Files) {
    ...
    if($taskDeletedCount % 1000 -eq 0){Write-Output ("DELETE PROGRESS: {0}/50638 files; {1} logical bytes" -f $taskDeletedCount,$taskDeletedBytes)}
}
...
if(-not(Test-Path -LiteralPath (Join-Path $taskExecutionRoot 'execution_authority_single_absence_final.json'))){Save-NewJson 'execution_authority_single_absence_final.json' $taskAuthority}
...
$result=[ordered]@{
 ...
 originally_approved_files=50639;originally_approved_logical_bytes=12122958520
 already_absent_files=1;already_absent_manifest_bytes=36178
 effective_delete_files=50638;effective_delete_logical_bytes=12122922342
 approved_files=50638;approved_logical_bytes=12122922342
 ...
 adapted_validator_sha256='e25f89256ee96578687bf91e73b81c9e163e35355a7da14f13f6746d444a3095'
 actual_deleted_files=$taskDeletedCount;actual_deleted_logical_bytes=$taskDeletedBytes;failed_operations=[int]($null -ne $taskFailure);remaining_approved_files=50638-$taskDeletedCount
 first_error=$taskFailure;postcheck=$taskPost
 failed_or_remaining_files=50638-$taskDeletedCount
...
Save-NewJson 'execution_result_single_absence_final.json' $result
[pscustomobject]@{classification=$classification;deleted_files=$taskDeletedCount;deleted_logical_bytes=$taskDeletedBytes;error=$taskFailure;remaining=50638-$taskDeletedCount}|ConvertTo-Json -Depth 5 -Compress
