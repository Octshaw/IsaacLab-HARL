...
> Authoritative Design V2.1 candidate: targeted contract correction over V2
...
supersedes:
  Lifecycle_Aware_Event_Gated_Local_MRTA_Design_Authoritative_V2_20260727.md

targeted_revision_basis:
  targeted GPT review after V2 revision
...
next_technical_phase:
  Phase A only, after user approval
```
...
均保持不变。V2.1 继承 V2 已解决的 DR-01～DR-21，本轮只关闭 targeted GPT review
提出的 TR-01～TR-03。完成本轮后不需要再次 broad review，但在用户最终确认前仍是
`AUTHORITATIVE-DESIGN-CANDIDATE-V2.1`，不授权 Phase A 或任何代码实现。
...
4. [Authoritative V2](Lifecycle_Aware_Event_Gated_Local_MRTA_Design_Authoritative_V2_20260727.md)；
5. [V2 revision summary](PHASE10A_AUTHORITATIVE_V2_REVISION_SUMMARY.md)；
6. [设计备忘录独立审核](../20260724/PHASE10A_EVENT_GATED_LOCAL_MRTA_DESIGN_BACKUP_REVIEW.md)；
7. [V2.1 targeted revision summary](PHASE10A_AUTHORITATIVE_V2_1_TARGETED_REVISION_SUMMARY.md)；
8. 当前 HEAD 中本文列出的 environment、wrapper、resolver、training 和 checkpoint 入口。
...
本次 targeted revision 指令明确要求的三个修正是 V2.1 的冻结目标。其余由 V2
冻结的方法契约不因本轮窄修订而改变；尚未实现的目标继续统一标记为
...
`RESOLVED` 在本文末尾 finding matrix 中只表示文档问题已被 V2/V2.1 修正，不表示代码
已实现、数值已确定、实验已完成或 V2.1 已获最终批准。
...
| transition authority | core path 不存在 authoritative raw/derived 双层对象 | immutable `ExecutionTransitionFacts` + `LifecycleTransitionResult` | A/B0 |
...
| playback | current path 每步逐 actor inference；诊断脚本有 coverage-only pre-reset monkey-patch，但不是 lifecycle authority | DVM-aware inference + authoritative facts/result chain | A/B0/C |
...
→ environment creates immutable ExecutionTransitionFacts before reset
→ lifecycle authority consumes ExecutionTransitionFacts
→ lifecycle authority updates completion/release/failed-pair/availability/ownership
→ lifecycle authority derives TEAM_INFEASIBLE
→ lifecycle authority determines termination reason
→ LifecycleTransitionResult is finalized
→ same-transition lifecycle events are collected from that result
→ if terminal:
     DVM = 0
     no proposal
     no transfer commit
     ...
     → per-robot Top-K
     → global observation + local mask + DVM snapshot
...
environment 只负责在可能的 `_reset_idx()` 前保存 raw execution facts；唯一 lifecycle
authority 负责派生 task/robot/ownership state、`TEAM_INFEASIBLE`、termination reason
和 lifecycle events。resolver 不负责推导 `TEAM_INFEASIBLE`，wrapper 也不得从 reset 后
state 反推 terminal transition。

`physical_terminated`、`physical_truncated` 和 `time_limit_reached` 可以出现在 raw facts；
`ALL_TASKS_COMPLETED`、`NO_FEASIBLE_TASKS_REMAIN` 和 `TIME_LIMIT` 等最终 lifecycle
reason 只能出现在 `LifecycleTransitionResult`。

仍须先更新事实，再让 policy 决策。不能使用旧 ownership、旧 failed pair 或 reset 后
状态生成本 tick 的 observation、mask、DVM 或 proposal。
...
- robot unavailable；
- robot recovered；
- execution/system/lifecycle authority 造成的 ownership material change；
- externally changed task/robot feasibility；
...
### 4.4 Ownership-change event provenance

**Status:** `[FROZEN-TARGET-NOT-IMPLEMENTED]`

assignment-triggering ownership/assignment change 包括：

- completion 导致 ownership 解除；
- terminal pair failure 导致 release；
- forced release；
- robot unavailable 或 recovered；
- externally changed feasibility；
- execution/system/lifecycle authority 改变 ownership；
- 仍有 `NEEDS_ASSIGNMENT` robot；
- configured retry cadence 到期。

以下变化本身不进入新的 assignment-triggering event set：

```text
ownership/effective-assignment change
produced by the immediately preceding successful resolver commit
```

它是当前 assignment tick 的输出，不能仅因 commit 成功就立即再生成一个 tick。冻结：

```text
resolver commit
→ update effective assignment/ownership
→ start next physical execution step

resolver commit alone
≠ new assignment-triggering lifecycle event
```

event provenance 至少使用：
...
`RESOLVER_COMMIT_DIAGNOSTIC` 只用于日志、state attribution 和 generation audit，不得
进入 assignment-triggering event set。commit 后只有存在以下至少一项独立条件，才调度后续
assignment opportunity：

- `NEEDS_ASSIGNMENT` robot；
- 新 execution/lifecycle event；
- retry cadence 到期；
- external availability/feasibility change；
- current component commit 显式留下 unresolved pending assignment。

### 4.5 Terminal rule

terminal branch 只能基于 finalized `LifecycleTransitionResult` 决定。raw
`ExecutionTransitionFacts` 只记录 physical termination/truncation/time-limit facts；
最终 lifecycle reason 不得提前混入 raw object。
...
1. seeds：
   ...
   ```text
   trigger-eligible lifecycle event robots
   ∪ NEEDS_ASSIGNMENT robots
   ```

   `RESOLVER_COMMIT_DIAGNOSTIC` provenance 不属于 trigger-eligible event；
...
- trigger-eligible ownership/candidate/availability/lifecycle event 或 retry schedule 到期后
  才重新激活；
- immediately preceding resolver commit 的 ownership change diagnostic 本身不重新激活。
...
进入 resolver 前，唯一 lifecycle authority 已消费 `ExecutionTransitionFacts`、完成
completion/release/failed-pair/availability/ownership 更新、推导
`TEAM_INFEASIBLE` 和 termination，并生成 `LifecycleTransitionResult`。resolver 只消费
与该 result 同 generation 的 event-updated baseline `a0`；它不得重新派生或重复应用
forced lifecycle facts。

每个非 terminal assignment tick：

1. 验证 `LifecycleTransitionResult`、decision snapshot、proposal 与 `a0` generation 一致；
2. 以 lifecycle authority 已更新的 ownership 形成/确认 event-updated baseline `a0`；
...
10. 接受后一次性 commit effective assignment 与 ownership，并只发出
    `RESOLVER_COMMIT_DIAGNOSTIC` provenance。
...
成功 commit 后立即进入下一 physical control step。commit 产生的 ownership change
本身不触发新的 assignment tick；只有 §4.4 列出的独立 trigger condition 仍成立时，才
安排下一 assignment opportunity。
...
`a0` 由唯一 lifecycle authority 在应用 completion、forced release/failure、availability、
failed-pair 和 `TEAM_INFEASIBLE` rules 后，写入 finalized
`LifecycleTransitionResult.updated_ownership`。resolver 对该 generation 的 baseline
只读；forced lifecycle release 不是 active proposal 导致的 assigned-count decrease。
...
`terminal_pair_failure_signals` 是 `ExecutionTransitionFacts` 中的 raw signal；
`new_failed_pairs`、对应 release 和后续 `TEAM_INFEASIBLE` 是 lifecycle authority 写入
`LifecycleTransitionResult` 的 derived result。resolver 不消费 raw signal，也不执行
这些推导。
...
该 reason 必须由 lifecycle authority 在 updated task state 和
`new_team_infeasible_tasks` 形成后确定，并写入 `LifecycleTransitionResult`；它不属于
raw `ExecutionTransitionFacts`。
...
- 每个 resolved pre-event-gated profile 分别保持自身既有 reward behavior；event-gated off
  不把 Contract C 降级为 resolver-disabled legacy；
- 本契约尚未实现。
...
## 22. Raw execution facts 与 derived lifecycle result
...
它不提供 `ExecutionTransitionFacts` / `LifecycleTransitionResult`、generation match
或 consume-once authority，不能作为训练接口的 raw-facts source。

### 22.2 `ExecutionTransitionFacts`

**Status:** `[FROZEN-TARGET-NOT-IMPLEMENTED]`

environment 必须在每个 physical transition、且在任何可能的 `_reset_idx()` 前生成
immutable raw facts：
...
- `completion_signals` 是 execution layer 报告的原始完成信号；
- `terminal_pair_failure_signals` 是结构性 terminal failure 原始信号；
- `forced_release_signals` 是 execution/system layer 明确报告的强制释放事实；
- `physical_terminated`、`physical_truncated`、`time_limit_reached` 和
  `bad_transition` 是 raw execution/runtime facts，不是最终 lifecycle reason；
- `coverage_before_reset` 与三类 `*_before_transition` 字段保存 transition 输入/执行后、
  lifecycle 派生前的权威 snapshot。

`ExecutionTransitionFacts` 禁止包含：

- `TEAM_INFEASIBLE`；
- 最终 `termination_reason`；
- lifecycle 更新后的 task/robot/ownership state；
- 基于所有 failed pairs 推导出的任何 result。

Authority 规则：

- snapshot immutable；
- environment 在 `_reset_idx()` 前完成 snapshot；
- 每份 raw facts 只能由唯一 lifecycle authority 消费一次；
- facts 与 consumer 的 episode/transition/env generation 必须精确匹配；
- stale、duplicate 或 future generation fail-fast；
- wrapper 不得用 reset 后 `get_assignment_problem()` 解释上一 transition；
- wrapper 不得自行从 raw facts 派生第二套 lifecycle state。

### 22.3 `LifecycleTransitionResult`

**Status:** `[FROZEN-TARGET-NOT-IMPLEMENTED]`

唯一 lifecycle authority 消费一份 `ExecutionTransitionFacts` 后生成：
...
- `completed_tasks` 在 `completion_signals` 应用到 lifecycle state 后产生；
- `released_tasks` 可以来自 forced release signal、terminal pair failure 后的 ownership
  release，或 robot unavailable 等 lifecycle rule；
- `new_failed_pairs` 只来自 authoritative structural terminal pair failure；
- `new_team_infeasible_tasks` 必须在 failed-pair state 更新后推导；
- `termination_reason` 必须在 updated task state 形成后决定；
- `lifecycle_events` 是 local MRTA seed/event collector 的唯一权威 transition input；
- `facts_consume_token` 证明对应 raw facts 已被唯一 authority consume，且 result 与 facts
  具有相同 generation。

source generation/token 缺失、不匹配或已被消费时，不得生成 result；duplicate、stale 或
future result 同样 fail-fast。

finalized result 同样 immutable。`updated_ownership` 是 policy/resolver 之前的
event-updated baseline `a0`；resolver commit 的 effective ownership 是独立输出，不能
回写、追加或事后修改已经 finalized 的同 transition result。

resolver 只读取 finalized `LifecycleTransitionResult` 与由它形成的 event-updated baseline；
不得自行推导 `TEAM_INFEASIBLE`、release 或 final termination。terminal result 使
DVM 全 0，并禁止 proposal/transfer commit。

### 22.4 Phase ownership

**Status:** `[FROZEN-TARGET-NOT-IMPLEMENTED]`

- Phase A 只定义、版本化 `ExecutionTransitionFacts` 与
  `LifecycleTransitionResult`，并冻结 generation、consume-once、authority 与 assertions；
  不接入真实 environment hook；
- Phase B0 由 environment 实现 pre-reset `ExecutionTransitionFacts` hook，由唯一
  lifecycle authority 消费 raw facts、生成 `LifecycleTransitionResult`，并实现 terminal
  pair failure、release、`TEAM_INFEASIBLE`、termination reason 与
  generation/consume-once tests。
...
最严格的 event-gated feature-off 契约：
...
supported pre-event-gated identities 可以包括：

```text
resolver-disabled legacy
lifecycle_contract_c
other already-versioned pre-event-gated profiles
```

event-gated off 只表示：

- 不进入新 DVM subset sampling；
- 不进入新 event/local-set/Top-K path；
- 不进入新 transfer-component resolver；
- 不进入新 team-reward/factor contract。

event-gated off 不表示：

- 强制切回 resolver-disabled legacy；
- 删除 Contract C lifecycle semantics；
- 改变 resolved current profile 的 resolver、mask、reward 或 checkpoint。

dispatcher 必须按 resolved profile 分派：

```text
resolved profile is legacy:
  use existing legacy path

resolved profile is lifecycle_contract_c:
  use existing Contract C path

resolved profile is another already-versioned pre-event-gated profile:
  use its existing profile-specific supported/blocked path

resolved profile is event_gated_local_mrta:
  use new Phase A–D path
```

每个既有 pre-event-gated profile 必须分别保持自己的：
...
- logger output；
- file side effects；
- checkpoint v2 identity；
- playback behavior。

禁止以：

```text
new event-gated trainer
+ DVM all ones
```

模拟任何旧 profile。

Event-gated feature-on：

- 使用新的高层 profile；
- 使用新的 semantic contract/fingerprint；
- 即使 tensor shape 相同，也不得把任何 pre-event-gated v2 当作正常 event-gated
  checkpoint；
...
- `TEAM_INFEASIBLE`；
- termination reasons；
- `ExecutionTransitionFacts` schema version；
- `LifecycleTransitionResult` schema version 与 authority contract；
- event-source taxonomy、trigger eligibility 和 resolver-commit trigger suppression。
...
- parser 不得单向改成只支持 v3；
- event-gated-off 时，v2 只在 exact resolved pre-event-gated semantic identity 下继续
  可用，包括已版本化的 legacy 或 `lifecycle_contract_c` identity；
...
- current resolver event drain 只进入 diagnostics/runtime payload；当前尚无 event-gated
  assignment-trigger loop；
...
Phase A 必须定义单一 resolved-profile dispatcher/identity，并使 scenario parse、apply、
runtime manifest、checkpoint 和 playback 使用同一 resolved identity。event-gated
default-off gate 不得覆盖或降级 dispatcher 选中的既有 profile。
...
- termination reason；
- event source 与 assignment-trigger eligibility；
- suppressed resolver-commit trigger count/assertion；
- proposal/effective mismatch；
- nondecision factor-identity assertion；
- team reward reducer/broadcast equality；
- raw-facts generation/consume-once assertion；
- lifecycle-result generation/authority-match assertion。
...
- event-gated default-off gate 与 resolved-profile dispatcher；
- 定义并版本化 `ExecutionTransitionFacts` 与 `LifecycleTransitionResult`；
- typed event provenance/cost/local-set/DVM/component DTO；
- generation、consume-once 与 authority identity；
- scenario parse/apply；
- checkpoint v2/v3 dispatcher；
- structured diagnostics；
- team reward contract config/assertions；
- 不改变 runtime behavior；
- 分别证明 supported pre-event-gated profile 的 feature-off identity。

### Phase B0 — Pre-reset facts / lifecycle transition

**Status:** `[FROZEN-TARGET-NOT-IMPLEMENTED]`

- environment pre-reset `ExecutionTransitionFacts` hook；
- lifecycle authority consume raw facts 并生成 `LifecycleTransitionResult`；
- terminal pair failure 与 release；
- `NEEDS_ASSIGNMENT`；
...
- trigger-eligible event seeds；
- event-source filter，排除 `RESOLVER_COMMIT_DIAGNOSTIC`；
- local set；
- owner expansion；
- overlap merge；
- Top-K；
...
- resolver-commit self-trigger suppression；
...
- 把 B0 authoritative termination result 接入 env/learner contract；
...
- resolver-disabled legacy feature-off 与其当前 HEAD path 的 observation/mask/action/
  log-prob/effective/reward/GAE/return/factor/RNG/side-effect identity；
- `lifecycle_contract_c` feature-off 与其当前 HEAD Contract C path 的同类 identity；
...
- `ExecutionTransitionFacts` 不含 derived fields；
- raw facts consume-once，result/facts generation 一一匹配；
- stale/duplicate/future generation fail-fast；
- failed-pair 更新后再派生 `TEAM_INFEASIBLE` 和 termination reason；
- commit-only ownership change 不产生下一 assignment tick；
- commit 后仍有 `NEEDS_ASSIGNMENT` 时 scheduler 可产生后续 opportunity；
- new execution event 仍正常触发，且 resolver-commit diagnostic 仍记录；
- old v2 semantic mismatch rejection in feature-on profile；
- each supported unchanged pre-event-gated profile 接受其 exact v2 semantic identity。
...
4. 阅读本文 V2.1；
5. 阅读 [V2.1 targeted revision summary](PHASE10A_AUTHORITATIVE_V2_1_TARGETED_REVISION_SUMMARY.md)；
6. 如需追溯，阅读已 supersede 的
   [V2](Lifecycle_Aware_Event_Gated_Local_MRTA_Design_Authoritative_V2_20260727.md)；
7. 执行：
...
8. 确认
   ...
   只读；
9. 确认最新 technical next step 是否仍为 Phase A；
10. 未经用户确认，不得训练、修改 resolver runtime behavior，或把 old checkpoint
    加载到新 profile。
...
| event-updated baseline | `LifecycleTransitionResult.updated_ownership` 表示的 resolver 前 baseline `a0` |
| `ExecutionTransitionFacts` | environment 在 reset 前保存的 immutable raw execution facts |
| `LifecycleTransitionResult` | 唯一 lifecycle authority 消费 raw facts 后形成的 immutable derived result |
| event source | `EXECUTION`、`LIFECYCLE_AUTHORITY`、`RETRY_SCHEDULER` 或 `RESOLVER_COMMIT_DIAGNOSTIC` |
| resolver-commit diagnostic | commit outcome 的非 triggering 诊断记录，不是下一 assignment tick 的独立 event |
...
V2.1 完整继承 V2 的下列 DR resolution；DR-01～DR-21 仍为 21/21 `RESOLVED`。
...
## 33. Targeted residual finding resolution matrix

`RESOLVED` 仍只表示本次 targeted 文档问题已关闭，不表示代码实现或用户批准。
...
## 34. Candidate approval gate

```text
targeted_revision_classification:
...
本轮已关闭 targeted GPT review 的三个 residual findings，无需再次 broad review。仍须等待
用户最终确认；在确认前不进入 Phase A，不运行训练、playback/evaluation，
